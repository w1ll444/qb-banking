...
window.addEventListener("message", function (event) {
    if(event.data.status == "openbank") {
        ...
        if (event.data.information.gender == 0){ /* Modificato qui */
            $(".h-profile_avatar").css({"background-image":"url('./images/male_avatar.png')"});
        }
        else if(event.data.information.gender == 1){
            $(".h-profile_avatar").css({"background-image":"url('./images/female_avatar.png')"});
        }
        ...
        let swooshreverse = new Audio('swooshreverse.mp3'); /* Modificato qui | Volume on close */
...
$(".btn, .nav-link").click(function()  { /* modificato qui */
...
function hicsintro(){
    const tl = gsap.timeline({ defaults: { ease: "power1.out" } }); /* Modificato qui */

    tl.to(".h-profileinfo", { y: "0%", duration: 2}); // Configurate duration of intro    
    tl.to(".intro", { opacity: "0", display: "none", duration: 0.8});  // Configurate duration disappear    
}

/* hicsintro();  */ // Active this for testing
